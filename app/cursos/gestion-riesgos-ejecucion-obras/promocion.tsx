...
const COUNTDOWN_DURATION_SECONDS = 10 * 60;
const PRODUCT_PRICE = 100;
const COUPON_REFRESH_INTERVAL_MS = 15000;
...
const Promocion = () => {
    ...
    useEffect(() => {
        ...
        // Reset coupons to total on page load
    ...
    // Simulate server-side coupon count (optional)
    useEffect(() => {
        if (!isClientHydrated) return;
    ...
    return (
        <div className="container mx-auto px-4 py-8 relative pt-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 items-center">
                <div className="bg-white p-2 rounded-lg shadow-md text-center flex flex-col items-center w-[500px] mx-auto">
                    ...
                    <div className="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-4 w-full max-w-md">
                        <button
                            ...
                            className="w-full sm:w-auto px-6 py-3 rounded-full bg-[#6e2c9a] text-white font-semibold hover:bg-purple-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        ...
                        <button
                            ...
                            className="w-full sm:w-auto px-6 py-3 rounded-full bg-[#00D6B7] text-white font-semibold hover:bg-green-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            disabled={!isClientHydrated || availableCoupons === 0 || timeLeft <= 0}
                        >
                            Plin
                        </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    <Image src="/yape.png" alt="Yape QR code or payment instruction visual" width={500} height={300} />
                    <p className="mt-2 text-sm text-gray-700 text-center font-semibold">ENVÍA TU COMPROBANTE DE PAGO Y DATOS AL NÚMERO DE WHATSAPP INDICADO.</p>
                </div>
            </div>
                ...
                {showConfirmationModal && (
                    ...
                    >
                        ...
                        >
                            ...
                            {currentPaymentMethod === 'Yape' && <p className="text-gray-500 text-sm mb-6">Envía constancia al Yape: [REEMPLAZAR CON TU NÚMERO YAPE]</p>}
                            {currentPaymentMethod === 'Plin' && <p className="text-gray-500 text-sm mb-6">Envía constancia al Plin: [REEMPLAZAR CON TU NÚMERO PLIN]</p>}
            ...
            <div className="mt-12 p-8 bg-white rounded-2xl shadow-xl text-gray-800 text-center max-w-2xl mx-auto border border-gray-200">
                <h2 className="text-2xl font-bold text-green-700 mb-4">¡Último paso importante!</h2>
                ...
                <div className="mt-6 flex flex-col sm:flex-row justify-center items-center gap-4">
                    <a href="https://wa.me/51916779050" target="_blank" rel="noopener noreferrer" className="bg-green-600 text-white font-semibold px-6 py-3 rounded-full shadow hover:bg-green-700 transition flex items-center gap-2" >
                    ...
                    <a href="https://wa.me/51922409224" target="_blank" rel="noopener noreferrer" className="bg-green-600 text-white font-semibold px-6 py-3 rounded-full shadow hover:bg-green-700 transition flex items-center gap-2" >
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"> <path d="M20.52 3.48A11.93 11.93 0 0 0 12 0a11.93 11.93 0 0 0-8.52 3.48A11.93 11.93 0 0 0 0 12c0 2.1.54 4.17 1.57 6.02L0 24l5.99-1.56A11.93 11.93 0 0 0 12 24c3.18 0 6.17-1.24 8.48-3.48A11.93 11.93 0 0 0 24 12c0-3.18-1.24-6.17-3.48-8.52zM12 21.82c-2.26 0-4.45-.66-6.3-1.9l-.45-.29-3.55.93.95-3.46-.3-.48A9.89 9.89 0 0 1 2.18 12c0-5.42 4.4-9.82 9.82-9.82S21.82 6.58 21.82 12 17.42 21.82 12 21.82zm5.6-7.08c-.28-.14-1.65-.82-1.9-.92s-.44-.14-.62.14c-.19.28-.72.92-.88 1.1-.16.18-.32.2-.6.07-.28-.14-1.18-.44-2.25-1.4-.83-.74-1.4-1.65-1.56-1.92s-.02-.42.12-.56c.13-.13.28-.32.42-.48.14-.16.18-.28.28-.46.09-.18.05-.34-.02-.48-.07-.14-.62-1.5-.85-2.06-.22-.53-.45-.46-.6-.46h-.52c-.18 0-.46.07-.7.34s-.92.9-.92 2.2.94 2.56 1.07 2.74c.14.18 1.86 2.84 4.5 3.98.63.27 1.12.43 1.5.55.63.2 1.2.17 1.65.1.5-.08 1.65-.67 1.88-1.32.23-.64.23-1.18.16-1.32-.06-.13-.26-.2-.54-.34z"/> </svg>
                        922 409 224
                    </a>
                </div>
