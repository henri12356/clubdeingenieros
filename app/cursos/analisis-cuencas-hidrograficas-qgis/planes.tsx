...
const planesData: Plan[] = [
  // Planes para EVENTOS (simulating basic/expert plans from image)
  {
    id: 'evento-general',
    ...
    oldPrice: 99.00,
    discountInfo: 'GRATIS',
    transmissionDates: '05, 06 de Mayo del 2025 en el horario de 7:00 PM a 9:00 PM.',
    features: [
      { text: 'Acceso a las 3 clases en vivo via zoom', included: true },
  ...
  },
  {
    ...
    transmissionDates: '05, 06, 09, 11 y 13 de Junio del 2025 en el horario de 7:00 PM a 9:00 PM.',
    features: [
      ...
      { text: 'Certificado Digital por 90 horas académicas con código QR', included: true },
    ],
  ...
  },
  {
    ...
    features: [
      ...
      { text: 'Soporte prioritario 24/7', included: true },
      { text: 'Descarga de contenido offline', included: true },
      { text: 'Funcionalidades adicionales para equipos', included: true },
    ],
    callToAction: 'Obtener Suscripción DUO',
    paymentOptions: {
      installments: 4,
      amountPerInstallment: Math.round(899 / 4), // Example installments
    },
    studentsInfo: 'Para 2 estudiantes',
  },
];

const Planes = () => {
  const [currentAudience, setCurrentAudience] = useState<'eventos' | 'suscripcion'>('eventos');
  ...
  const getWhatsAppLink = (plan: Plan) => {
    ...
    switch (plan.id) {
      case 'evento-general':
        message = '¡Hola! Estoy interesado/a en registrarme gratis en el curso de ' + plan.name + '.';
        break;
      case 'evento-vip':
        message = '¡Hola! Estoy interesado/a en adquirir la Entrada VIP para el curso de ' + plan.name + '.';
        break;
      case 'suscripcion-pro':
        message = '¡Hola! Estoy interesado/a en la Suscripción Anual PRO.';
        break;
      case 'suscripcion-duo':
        message = '¡Hola! Estoy interesado/a en la Suscripción Anual DUO.';
        break;
      default:
        message = '¡Hola! Me gustaría obtener más información sobre el plan ' + plan.name + '.';
    }
      ...
      <div className="max-w-7xl mx-auto text-center">
        ...
        >
          <button
            ...
            className={`px-5 py-2 sm:px-6 sm:py-2.5 rounded-md text-sm sm:text-base font-semibold transition-colors duration-300 relative z-10 whitespace-nowrap
              ${currentAudience === 'eventos' ? 'text-white' : 'text-gray-600 hover:text-red-600'}`}
            role="tab"
          ...
          <button
            ...
            className={`px-5 py-2 sm:px-6 sm:py-2.5 rounded-md text-sm sm:text-base font-semibold transition-colors duration-300 relative z-10 whitespace-nowrap
              ${currentAudience === 'suscripcion' ? 'text-white' : 'text-gray-600 hover:text-red-600'}`}
            role="tab"
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className={`grid grid-cols-1 gap-6 sm:gap-8 lg:gap-10 items-stretch justify-center ${currentAudience === 'eventos' ? 'md:grid-cols-2 lg:grid-cols-2 max-w-4xl mx-auto' : 'md:grid-cols-2 lg:grid-cols-3 max-w-full mx-auto'}`}
          >
            {filteredPlanes.map((plan) => (
              <motion.div
                ...
                className={`
                  ...
                  ${(plan.id === 'evento-vip' || plan.id === 'suscripcion-pro' || plan.id === 'suscripcion-duo') ? 'border-red-600 ring-2 ring-red-600' : ''}
                  ${(plan.id === 'suscripcion-pro' || plan.id === 'suscripcion-duo') ? 'lg:scale-105 lg:z-10' : ''}
                `}
              >
                ...
                <div className="text-left mb-4 sm:mb-6">
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-1 leading-tight">{plan.name}</h3>
                  <div className="flex items-center text-sm text-gray-600">
                    {plan.studentsInfo && <span className="mr-2">{plan.studentsInfo}</span>}
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">
                      {plan.billingCycle === 'unica' ? 'Pago Único' : (plan.billingCycle === 'anual' ? 'Anual' : 'Mensual')}
                    </span>
                  </div>
                  {plan.transmissionDates && (
                    <p className="text-gray-600 text-xs mt-2 leading-relaxed">
                      <span className="font-semibold">Fechas:</span> {plan.transmissionDates}
                    </p>
                  )}
                </div>
                ...
                <div className="mb-4 sm:mb-6 flex items-baseline justify-start">
                  <span className={`text-4xl sm:text-5xl font-extrabold ${getPriceTextColor(plan.type)}`}>
                    {plan.currency}
                    {plan.price.toLocaleString('es-PE', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                  </span>
                  {plan.billingCycle === 'anual' && (
                    <span className="text-gray-600 text-lg sm:text-xl ml-2">/año</span>
                  )}
                  {plan.billingCycle === 'mensual' && (
                    <span className="text-gray-600 text-lg sm:text-xl ml-2">/mes</span>
                  )}
                  {plan.oldPrice !== undefined && plan.oldPrice > plan.price && (
                    <span className="text-gray-500 line-through text-base sm:text-lg ml-2 sm:ml-3">
                      {plan.currency}
                      {plan.oldPrice.toLocaleString('es-PE', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                    </span>
                  )}
                </div>
                {plan.billingCycle === 'unica' ? (
                  <p className="text-gray-600 text-xs sm:text-sm mb-4 sm:mb-6">Acceso de por vida al contenido adquirido.</p>
                ) : (
                  <p className="text-gray-600 text-xs sm:text-sm mb-4 sm:mb-6">Acceso por un año a todo el contenido.</p>
                )}

                {/* Features List */}
                <ul className="text-left space-y-3 sm:space-y-4 flex-grow mb-6 sm:mb-8">
                  {plan.features.map((feature, index) => (
                    <li key={index} className={`flex items-start ${feature.included ? 'text-gray-700' : 'text-gray-400 line-through'}`}>
                      {feature.included ? (
                      ...
                      <span className="text-sm sm:text-base leading-relaxed">{feature.text}</span>
                    </li>
                  ))}
                </ul>
                ...
                {plan.paymentOptions && (
                  <div className="bg-gray-100 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6 text-center text-xs sm:text-sm text-gray-700">
                    <p className="leading-relaxed">
                      Paga hasta en <span className="font-bold">{plan.paymentOptions.installments} cuotas</span> sin intereses de <span className="font-bold text-red-600">
                        {plan.currency}
                        {plan.paymentOptions.amountPerInstallment.toLocaleString('es-PE', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                      </span>
                    </p>
                  </div>
                )}
                ...
                <a
                  ...
                  className={`w-full py-3 sm:py-4 rounded-lg text-base sm:text-lg font-bold text-white transition-colors duration-300 shadow-lg focus:outline-none focus:ring-2 ${getButtonBgColor(plan.type)} flex items-center justify-center`}
                >
                  {plan.callToAction}
                </a>
              </motion.div>
