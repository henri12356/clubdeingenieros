...
const Promocion = () => {
    const [availableCoupons, setAvailableCoupons] = useState(PROMOTION_TOTAL_COUPONS);
    ...
    useEffect(() => {
        ...
        localStorage.setItem('availableCoupons', PROMOTION_TOTAL_COUPONS.toString());
        localStorage.setItem('availableCoupons_server', PROMOTION_TOTAL_COUPONS.toString()); // Also reset server simulation
    }, []);
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(() => {
            ...
            const savedCoupons = localStorage.getItem('availableCoupons_server') || PROMOTION_TOTAL_COUPONS.toString();
            const serverCount = Math.min(PROMOTION_TOTAL_COUPONS, Math.max(0, parseInt(savedCoupons, 10)));
    ...
    return (
        <div className="container mx-auto px-4 py-8 relative pt-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 items-center">
                ...
                <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md text-center flex flex-col items-center w-full max-w-sm mx-auto"> {/* Added w-full max-w-sm */}
                    ...
                    <div className="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-4 w-full px-2"> {/* Added w-full px-2 for buttons */}
                        <button
                            ...
                            disabled={!isClientHydrated || availableCoupons === 0 || timeLeft <= 0}
                        >
                            Yape
                        </button>
                        <button
                ...
                <div className="bg-white p-6 rounded-lg shadow-md text-center">
                    ...
                    <p className="text-gray-600 mt-6 text-xl md:text-2xl font-bold">
                        INVERSIÓN: <span className="text-purple-600">S/.{PRODUCT_PRICE}</span> <span className="text-sm">(Único Pago)</span>
                    </p>
                    <div className="mt-4 text-left text-gray-700 text-sm max-w-xs mx-auto"> {/* Adjusted max-width for benefit list */}
                    ...
                    <p className="text-gray-500 text-xs mt-4 flex items-center justify-center text-center">
                        <span className="text-yellow-500 mr-1">▲</span> Una vez realizado el Yape, Transferencia, envíe al WhatsApp: <a href="https://wa.me/51922900001" target="_blank" rel="noopener noreferrer" className="text-blue-500 ml-1 hover:underline">+51 922900001</a>
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    ...
                    <h3 className="font-bold text-gray-700 mb-2">CUENTAS BANCARIAS:</h3> {/* Changed from 'FORMAS DE PAGO' for clarity */}
                    <div className="mb-4">
                ...
                {/* Yape QR Code Image */}
                <div className="flex flex-col items-center justify-center p-4"> {/* Added flex-col and justify-center for better alignment */}
                    <Image src="/yape.png" alt="Yape QR code or payment instruction visual" width={500} height={300} className="w-full h-auto max-w-sm rounded-lg shadow-md" /> {/* Added max-w-sm, rounded-lg, shadow-md */}
                    <p className="mt-4 text-sm text-gray-700 text-center font-semibold max-w-sm">ENVÍA TU COMPROBANTE DE PAGO Y DATOS AL NÚMERO DE WHATSAPP INDICADO.</p>
                </div>
            </div>
                ...
                {showConfirmationModal && (
                    ...
                    >
                        ...
                        >
                            ...
                            > &times; </button>
                            {currentPaymentMethod === 'Yape' && (<img src="https://www.yape.com.pe/images/logo-yape_positive.png" alt="Yape Logo" className="w-24 h-auto mx-auto mb-4" />)}
                            {currentPaymentMethod === 'Plin' && (<img src="https://plin.pe/wp-content/themes/plin/imgs/logo.png" alt="Plin Logo" className="w-24 h-auto mx-auto mb-4" />)}
                            {currentPaymentMethod === 'Transferencia' && (<p className="text-gray-500 text-sm mb-4">Transferencia bancaria</p>)}
                            <h3 className="text-lg font-semibold text-gray-800 mb-2">Confirmación de pago vía {currentPaymentMethod}</h3>
                            <p className="text-gray-600 mb-1">Cupo #{couponNumberForModal} reservado</p>
                            <p className="text-green-600 font-bold text-2xl mb-4">S/. {PRODUCT_PRICE}</p>
                            {currentPaymentMethod === 'Yape' && <p className="text-gray-500 text-sm mb-6">Envía constancia al Yape: <span className="font-semibold">[REEMPLAZAR CON TU NÚMERO YAPE]</span></p>}
                            {currentPaymentMethod === 'Plin' && <p className="text-gray-500 text-sm mb-6">Envía constancia al Plin: <span className="font-semibold">[REEMPLAZAR CON TU NÚMERO PLIN]</span></p>}
                            {currentPaymentMethod === 'Transferencia' && <p className="text-gray-500 text-sm mb-6">Realiza la transferencia a las cuentas indicadas.</p>}
                            <p className="text-gray-500 text-xs mb-4">No olvides enviar tu comprobante de pago y datos personales al WhatsApp indicado en la página.</p>
                            <button
                                onClick={confirmPurchase}
                                className="w-full px-6 py-3 rounded-full bg-purple-600 text-white font-semibold hover:bg-purple-700 transition-colors"
                            > Aceptar y Confirmar Reserva </button>
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <div className="mt-12 p-6 sm:p-8 bg-white rounded-2xl shadow-xl text-gray-800 text-center max-w-2xl mx-auto border border-gray-200">
                <h2 className="text-xl md:text-2xl font-bold text-green-700 mb-4">¡Último paso importante!</h2>
                <p className="text-base leading-relaxed"> Una vez realizado el depósito o transferencia, envía la constancia de pago al WhatsApp junto con tus datos personales: </p>
                <ul className="text-left mt-4 space-y-2 text-sm sm:text-base list-disc list-inside mx-auto max-w-xs">
                    ...
                    <li><strong>N° DNI</strong></li>
                    <li><strong>Correo Electrónico</strong></li>
                    <li><strong>N° Celular</strong></li>
                </ul>
                <div className="mt-6 flex flex-col sm:flex-row justify-center items-center gap-4">
                    <a href="https://wa.me/51916779050" target="_blank" rel="noopener noreferrer" className="bg-green-600 text-white font-semibold px-6 py-3 rounded-full shadow hover:bg-green-700 transition flex items-center gap-2 text-sm md:text-base" >
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"> <path d="M20.52 3.48A11.93 11.93 0 0 0 12 0a11.93 11.93 0 0 0-8.52 3.48A11.93 11.93 0 0 0 0 12c0 2.1.54 4.17 1.57 6.02L0 24l5.99-1.56A11.93 11.93 0 0 0 12 24c3.18 0 6.17-1.24 8.48-3.48A11.93 11.93 0 0 0 24 12c0-3.18-1.24-6.17-3.48-8.52zM12 21.82c-2.26 0-4.45-.66-6.3-1.9l-.45-.29-3.55.93.95-3.46-.3-.48A9.89 9.89 0 0 1 2.18 12c0-5.42 4.4-9.82 9.82-9.82S21.82 6.58 21.82 12 17.42 21.82 12 21.82zm5.6-7.08c-.28-.14-1.65-.82-1.9-.92s-.44-.14-.62.14c-.19.28-.72.92-.88 1.1-.16.18-.32.2-.6.07-.28-.14-1.18-.44-2.25-1.4-.83-.74-1.4-1.65-1.56-1.92s-.02-.42.12-.56c.13-.13.28-.32.42-.48.14-.16.18-.28.28-.46.09-.18.05-.34-.02-.48-.07-.14-.62-1.5-.85-2.06-.22-.53-.45-.46-.6-.46h-.52c-.18 0-.46.07-.7.34s-.92.9-.92 2.2.94 2.56 1.07 2.74c.14.18 1.86 2.84 4.5 3.98.63.27 1.12.43 1.5.55.63.2 1.2.17 1.65.1.5-.08 1.65-.67 1.88-1.32.23-.64.23-1.18.16-1.32-.06-.13-.26-.2-.54-.34z"/> </svg>
                        +51 916 779 050
                    </a>
                    <a href="https://wa.me/51922409224" target="_blank" rel="noopener noreferrer" className="bg-green-600 text-white font-semibold px-6 py-3 rounded-full shadow hover:bg-green-700 transition flex items-center gap-2 text-sm md:text-base" >
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"> <path d="M20.52 3.48A11.93 11.93 0 0 0 12 0a11.93 11.93 0 0 0-8.52 3.48A11.93 11.93 0 0 0 0 12c0 2.1.54 4.17 1.57 6.02L0 24l5.99-1.56A11.93 11.93 0 0 0 12 24c3.18 0 6.17-1.24 8.48-3.48A11.93 11.93 0 0 0 24 12c0-3.18-1.24-6.17-3.48-8.52zM12 21.82c-2.26 0-4.45-.66-6.3-1.9l-.45-.29-3.55.93.95-3.46-.3-.48A9.89 9.89 0 0 1 2.18 12c0-5.42 4.4-9.82 9.82-9.82S21.82 6.58 21.82 12 17.42 21.82 12 21.82zm5.6-7.08c-.28-.14-1.65-.82-1.9-.92s-.44-.14-.62.14c-.19.28-.72.92-.88 1.1-.16.18-.32.2-.6.07-.28-.14-1.18-.44-2.25-1.4-.83-.74-1.4-1.65-1.56-1.92s-.02-.42.12-.56c.13-.13.28-.32.42-.48.14-.16.18-.28.28-.46.09-.18.05-.34-.02-.48-.07-.14-.62-1.5-.85-2.06-.22-.53-.45-.46-.6-.46h-.52c-.18 0-.46.07-.7.34s-.92.9-.92 2.2.94 2.56 1.07 2.74c.14.18 1.86 2.84 4.5 3.98.63.27 1.12.43 1.5.55.63.2 1.2.17 1.65.1.5-.08 1.65-.67 1.88-1.32.23-.64.23-1.18.16-1.32-.06-.13-.26-.2-.54-.34z"/> </svg>
                        922 409 224
                    </a>
                </div>
